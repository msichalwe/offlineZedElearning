...
Future<List<LessonGradeCountRow>> performGetLessonGradeCount(
  Database database, {
  int? gradeId,
}) async {
  final query = '''
...
FROM grades g
JOIN syllabi s ON g.grade_id = s.grade_id
JOIN topics t ON s.syllabus_id = t.syllabus_id
JOIN subtopics st ON t.topic_id = st.topic_id
JOIN lessons l ON st.subtopic_id = l.outcome_id
WHERE g.grade_id = $gradeId ;
  ''';
